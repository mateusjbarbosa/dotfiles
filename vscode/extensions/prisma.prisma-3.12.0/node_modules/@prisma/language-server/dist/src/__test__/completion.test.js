...
function assertCompletion(fixturePath, position, expected) {
    ...
    assert.deepStrictEqual(completionResult.isIncomplete, expected.isIncomplete, `Line ${position.line} - Character ${position.character}
Expected isIncomplete to be '${expected.isIncomplete}' but got '${completionResult.isIncomplete}'`);
    assert.deepStrictEqual(completionResult.items.map((item) => item.label), expected.items.map((item) => item.label), `Line ${position.line} - Character ${position.character}
mapped items => item.label`);
    assert.deepStrictEqual(completionResult.items.map((item) => item.kind), expected.items.map((item) => item.kind), `Line ${position.line} - Character ${position.character}
mapped items => item.kind`);
    assert.deepStrictEqual(completionResult.items.length, expected.items.length, `Line ${position.line} - Character ${position.character}
...
suite('Completions', function () {
    ...
    suite('TYPES', () => {
        ...
        test('Diagnoses type suggestions in model block - MongoDB', () => {
            assertCompletion(mongoDBFieldTypes, { line: 11, character: 14 }, {
                isIncomplete: true,
                items: [
                    { label: 'String', kind: vscode_languageserver_1.CompletionItemKind.TypeParameter },
                    { label: 'Boolean', kind: vscode_languageserver_1.CompletionItemKind.TypeParameter },
    ...
    suite('FIELD ATTRIBUTES', () => {
        ...
        suite('MongoDB', function () {
            test('String @id @default(|)', () => {
                assertCompletion(mongoDBAtdefaultUri, { line: 11, character: 33 }, {
                    isIncomplete: false,
                    items: [functionAuto, functionUuid, functionCuid],
                });
            });
            test('Int @id @default(|)', () => {
                assertCompletion(mongoDBAtdefaultUri, { line: 24, character: 22 }, {
                    isIncomplete: false,
                    items: [functionAuto, functionAutoincrement],
                });
            });
            test('String @default(|)', () => {
                assertCompletion(mongoDBAtdefaultUri, { line: 12, character: 29 }, {
                    isIncomplete: false,
                    items: [functionAuto, functionUuid, functionCuid],
                });
            });
            test('Boolean @default(|)', () => {
                assertCompletion(mongoDBAtdefaultUri, { line: 13, character: 29 }, {
                    isIncomplete: false,
                    items: [functionAuto, staticValueTrue, staticValueFalse],
                });
            });
            test('DateTime @default(|)', () => {
                assertCompletion(mongoDBAtdefaultUri, { line: 14, character: 29 }, {
                    isIncomplete: false,
                    items: [functionAuto, functionNow],
                });
            });
            test('Enum @default(|)', () => {
                assertCompletion(mongoDBAtdefaultUri, { line: 15, character: 29 }, {
                    isIncomplete: false,
        ...
        suite('Diagnoses arguments of @relation directive', function () {
            ...
            test('MongoDB: embedded m2m @relation(references: [|])', () => {
                assertCompletion(mongoDBEmbeddedM2mUri, { line: 13, character: 58 }, {
                    isIncomplete: false,
                    items: [
                        { label: 'id', kind: vscode_languageserver_1.CompletionItemKind.Field },
